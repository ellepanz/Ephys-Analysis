function BinderIndex = MINIS_setPopulationInclusion(Expt,includeInPopulation,reason,dataFile,figureFolder)
% Explicitly mark one analyzed MINIS cell in/out of its study population.
%
% Example:
%   MINIS_setPopulationInclusion(Expt,false, ...
...
includeInPopulation = logical(includeInPopulation);
...
elseif strlength(reason) == 0
    error('Give an exclusion reason when IncludeInPopulation=false.');
end
...
fprintf('%s IncludeInPopulation = %d\n',Expt.marker,includeInPopulation);
