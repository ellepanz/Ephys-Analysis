...
% Plot 1-s holding-current and phasic-current variability for every analyzed
% condition except NMDA.
%
...
if nCond == 0
    error('No conditions remain for holding/phasic variability plotting.');
end
...
for c = 1:nCond
    ...
    if ~isempty(epochValues)
        x = c + linspace(-0.15,0.15,numel(epochValues))';
        scatter(axPhasic,x,epochValues,15,plotColors{c},'filled','MarkerFaceAlpha',0.25);
    end

    trialMeans = mean(values,1,'omitnan');
    validTrialMeans = trialMeans(isfinite(trialMeans));

    scatter(axPhasic,repmat(c,size(validTrialMeans)),validTrialMeans,55,plotColors{c}, ...
        'filled','MarkerEdgeColor','k');

    phasicMeans(c) = mean(validTrialMeans,'omitnan');
    plot(axPhasic,[c-0.25 c+0.25],[phasicMeans(c) phasicMeans(c)],'k-','LineWidth',2);
end

yline(axPhasic,0,':');
set(axPhasic,'XTick',1:nCond,'XTickLabel',strrep(plotConditions,'_','/'));
xlim(axPhasic,[0.5 nCond+0.5]);
ylabel(axPhasic,'Phasic I_{mean} (pA)');
title(axPhasic,'Phasic current');
box(axPhasic,'off');

sgtitle(sprintf('%s Summary',Expt.marker));

% Print useful within-cell changes relative to the preceding analyzed condition.
for c = 2:nCond
    fprintf('%s -> %s: holding delta %.2f pA; phasic delta %.2f pA\n', ...
        strrep(plotConditions{c-1},'_','/'),strrep(plotConditions{c},'_','/'), ...
        holdingMeans(c)-holdingMeans(c-1),phasicMeans(c)-phasicMeans(c-1));
end

if nargin >= 5 && ~isempty(figureFolder)
    savefig(fig,fullfile(figureFolder,'Phasic Holding Current Variability.fig'));
    exportgraphics(fig,fullfile(figureFolder,'Phasic Holding Current Variability.png'),'Resolution',300);
end
